...
nireq=6
nstreams=6
result_index = [0] * nireq 
result_value = [0] * nireq 

def completion_callback(infer_request, ireq):
    result = infer_request.get_output_tensor()
    result = np.squeeze(result.data)
    result_index[ireq] = np.argmax(result)
    result_value[ireq] = result[result_index[ireq]] 
...
compiled_model = core.compile_model(model=model, device_name="CPU", config={"NUM_STREAMS": nstreams})
infer_queue = AsyncInferQueue(compiled_model, nireq)
...
infer_queue.start_async({0: input_tensor}, 0)
...
print(f"Result: {imagenet_classes[result_index[0]]} {result_value[0]}")
...
for _ in range(num_images):
    input_tensor = np.expand_dims(image_orig, 0)
    ireq_idle = infer_queue.get_idle_request_id()
    infer_queue.start_async({0: input_tensor}, ireq_idle)
